...
is the query named "GetUserOrders",
which can be accessed via the [ExampleConnector.getUserOrders] property.


An example of the property for a mutation
is the mutation named "CreateReview",
which can be accessed via the [ExampleConnector.createReview] property.
...
For example, the "ListMeals" query has no variables
and can be executed via the
[ExampleConnector.listMeals]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.listMeals.execute()
println("ListMeals query returned: ${queryResult.data}")
```
...
For example, the "GetUserOrders" query has 1 required variable ("userId")
and can be executed via the [ExampleConnector.getUserOrders]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.getUserOrders.execute(userId=java.util.UUID.randomUUID())
println("GetUserOrders query returned: ${queryResult.data}")
```
...
For example, the "CreateReview" mutation has 3 required variables ("userId", "mealId", and "rating")
and can be executed via the [ExampleConnector.createReview]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.createReview.execute(userId=java.util.UUID.randomUUID(), mealId=java.util.UUID.randomUUID(), rating=7324)
println("CreateReview mutation returned: ${mutationResult.data}")
```
...
For example, the "UpdateOrder" mutation has 1 optional variable ("status")
and can be executed via the [ExampleConnector.updateOrder]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.updateOrder.execute(id=java.util.UUID.randomUUID()) {
  status = "garply"
}
println("UpdateOrder mutation returned: ${mutationResult.data}")
```

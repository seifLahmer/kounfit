...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const unsubscribeSnapshot = onSnapshot(userDocRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        // ✅ Si JSON existe ET région différente => reset
        if (savedData && userData.region && prevRegion && prevRegion !== userData.region) {
          localStorage.removeItem(`dailyPlanData_${firebaseUser.uid}`)
          setUnconfirmedPlan(emptyPlan)
          toast({
            title: "Région modifiée",
            description: "Vos statistiques ont été réinitialisées.",
          })
        }

        // Toujours mettre à jour la région stockée
        if (userData.region) {
          localStorage.setItem(`region_${firebaseUser.uid}`, userData.region)
        }

        // Repas confirmés du jour
  ...
  const consumedMacros = allMealsForToday.reduce(
    (acc, meal) => {
      if (meal && meal.macros) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-primary pb-8">
      <header className="flex-shrink-0 pt-4 pb-4"></header>
        ...
        <CardContent className="p-4 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <MealCard
              ...
              meals={getMealsForCategory("breakfast")}
              onAdd={() => handleAddMeal("breakfast")}
              defaultImage="/img home/petit-dejeuner.png"
            />
            <MealCard
              title="Déjeuner"
              meals={getMealsForCategory("lunch")}
              onAdd={() => handleAddMeal("lunch")}
              defaultImage="/img home/dejeuner.png"
            />
            <MealCard
              title="Dîner"
              meals={getMealsForCategory("dinner")}
              onAdd={() => handleAddMeal("dinner")}
              defaultImage="/img home/dinner.png"
            />
            <MealCard
              title="Collation"
              meals={getMealsForCategory("snack")}
              onAdd={() => handleAddMeal("snack")}
              defaultImage="/img home/snacks.png"
            />
          </div>
        </CardContent>
      </Card>
